...
import com.into.websoso.domain.model.NicknameValidationResult.INVALID_NICKNAME_DUPLICATION
import com.into.websoso.domain.model.NicknameValidationResult.NETWORK_ERROR
import com.into.websoso.domain.model.NicknameValidationResult.NONE
import com.into.websoso.domain.model.NicknameValidationResult.UNKNOWN_ERROR
...
import com.into.websoso.ui.mapper.toProfileEdit
...
class ProfileEditViewModel
    ...
    constructor(
        ...
        fun updateSelectedGenres(selectedGenre: Genre) {
            ...
            if (genrePreferences.contains(selectedGenre)) {
                genrePreferences.remove(selectedGenre)
            } else {
                genrePreferences.add(selectedGenre)
            }
        ...
        private fun isNicknameChanged(nickname: String): Boolean =
            when (
                nickname == profileEditUiState.value.let {
            ...
            viewModelScope.launch {
                runCatching {
                    userRepository.saveEditingUserProfile(
                        ...
                        nickname = (previousProfile.nicknameModel.nickname to currentProfile.nicknameModel.nickname)
                            .compareAndReturnNewOrNullValue(),
        ...
        fun updateCheckDuplicateNicknameButtonEnabled() {
            val isEnable =
                profileEditUiState.value
                    ...
                    ?.nickname
                    ?.isNotEmpty() == true &&
                    profileEditUiState.value?.nicknameEditResult == NONE
        ...
        fun getRepresentativeAvatar(): AvatarModel =
            profileEditUiState.value?.profile?.let { profile ->
                avatarChangeUiState.value?.avatars?.find { it.avatarId == profile.avatarId }
            } ?: AvatarModel()

        fun getFormattedSpanCount(): Int =
            avatarChangeUiState.value?.avatars?.size?.let { avatarCount ->
                when {
